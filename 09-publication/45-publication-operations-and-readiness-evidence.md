...
- All five fenced text diagrams receive source-derived accessible names from their nearest headings while preserving their complete text. Rendered validation requires a consistently labelled diagram group and code block; print exposes the generated caption.
- Production validation emits `.astro/performance-report.json` and blocks regressions in total artifact size, HTML P95 and maximum, search index, CSS, JavaScript, fonts, raster images, immutable asset identity, and representative script-independent content. Exact initial thresholds are recorded in implementation record 53.
- Search provides a client-local unavailable-index validation switch. The real failure branch announces the problem, disables its unusable input, and links to generated journeys, decisions, and coverage without changing server state or collecting a query.
- Chrome 149 browser-print inspection passed short, long, 59-page tabular, 15-page diagram-heavy, and technical documents after correcting orphaned table headings and separated diagram captions. All samples retained print identity and readable, unclipped, theme-independent content; rendered CSS validation preserves the pagination controls.
...
- CI uses locked installation, read-only repository permission, immutable artifact packaging, a digest, concurrency control, and bounded retention. The artifact includes the Blueprint manifest, coverage report, and performance report generated by the same validated build.
- A secretless GitHub Actions monitor checks the temporary canonical origin every six hours and on demand, retains structured pass/fail diagnostics, and exercises representative routes, landmarks, headers, search coverage, and the real 404 path.
- CI rehearses staged-artifact corruption detection and exact-tree restoration, validates routing and security controls, and packages the resulting tree digest and recovery report with the release candidate.
- A machine-readable final-readiness audit packages every safe pass and every human, external, authority, or controlled-value blocker; it cannot report completion while any blocker remains.
...
- Native Vercel production deployment `dpl_2VdSf8FiuxenUYMkW1PwP85ekAUD` reached READY from Git commit `c80e72d258b12bb3c277fba026711b4c0b07dff4`. Canonical-alias smoke checks passed six routes, the 286-entry search index then generated by that commit, CSP without WebAssembly evaluation exceptions, defensive headers, indexing isolation, and custom 404 behavior.
- Hosted browser checks passed exact ADR identifier search with `Accepted`, authority, and repository-path context; the Portuguese term `português`; empty-result guidance; input focus and keyboard close with focus return; and 390 CSS-pixel reflow without document, dialog, or result overflow. A fresh-tab load emitted no console errors. Explicit runtime fault injection for the unavailable-index recovery state remains pending; rendered validation already requires its three generated recovery routes and the no-script equivalent.
- Hosted deployment `dpl_veoUV3Z39fWHhiSQWDYxQg4ziV1a`, correlated with commit `25cf46c4f0e75afeaae891aeb5863705fa1dce89`, passed table semantics and 390 CSS-pixel reflow checks: accessible caption/name, row and column header roles, labelled focusable local-scroll region, zero document overflow, and no fresh console errors.
- Hosted deployment `dpl_1Hh4mAwnvQp6gPvbhzTmQpmWX7CG`, correlated with commit `f68f475a59bba31d152ad33bde31b6154b6db165`, passed representative long-document and 404 checks at 320 CSS pixels, light/dark theme switching, mobile-menu open and Escape focus return, named error recovery links, zero document overflow, and fresh-console inspection. Full Tab-sequence and visible-focus evidence remains open because the browser driver's synthetic body-level Tab did not create a reliable focus transition.
- Hosted deployment `dpl_6aiCZ6mjgVWGZnJ67hv82HnJgKWa`, correlated with commit `1542a48391455d3016ef9d00484b08a2ac4a65d4`, exposed the representative topology as a named text-diagram group with preserved text and zero document, group, or code overflow at 1280 CSS pixels. Hosted viewport resizing was unavailable for this run, so narrow-width diagram evidence remains open.
- Hosted deployment `dpl_C8dxA5fmTqj4gWcm3S15ARKr1ppS`, correlated with commit `3ddba5c4238dbb7a141213006cb29dc4646c1fba`, passed explicit unavailable-search fault injection: announced failure, disabled input, three named recovery links, zero document overflow, and successful navigation to the generated journeys view.
- GitHub Actions monitor run `32213726794` passed the canonical-origin smoke contract from commit `7e5f464f0c7c61f89e393501326911dac8126684` and retained digest-recorded report artifact `9351624088`. Correlated Publication CI run `32213718588` passed every build, audit, and packaging step and retained immutable artifact `9351632523`; both used Node 24 action majors without the prior Node 20 deprecation annotation.
- Read-only Vercel inspection confirmed deployment `dpl_8u3Dgmf74LgMdR3jTN68zjVvnoWY` READY from commit `720f16cd028846ffda97486a222b49a33499db16`, with no alias error, two available rollback candidates, successful locked build evidence, and no aggregated runtime errors over seven days. No production alias or provider setting was changed.
...
- The temporary `nexora-blueprint.vercel.app` origin has passed baseline route, header, indexing-isolation, landmark, overflow, 404, CSP-safe search, status-context, Portuguese-term, empty-result, keyboard-focus, and 390 CSS-pixel checks.
- Vercel Git integration, project settings, protected deployment values, hosted smoke monitoring, provider build logs, and runtime-error aggregation are inspected. Independent second-network monitoring and custom DNS remain unaccepted.
...
- Artifact-level corruption detection and exact restoration now pass in CI. Provider-specific immutable promotion, alias rollback, outage fallback, DNS recovery, and access-recovery rehearsals still require production authority.
...
- Manual screen-reader, forced-color, explicit browser zoom, constrained-network, and disabled-user evidence is not yet signed off. Representative print and search-failure evidence now pass.
